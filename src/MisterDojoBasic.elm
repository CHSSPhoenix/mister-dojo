...
subscriptions model =
    ...
    in
        if model.isThereARoundActive then
            Time.every second (\_ -> launchAt model.minutesCounter)
        else
            Sub.none
            ...
            ( { model
                ...
                , devList =
                    if String.isEmpty model.temporalDev.name then
                        ...
                        in
                            Dict.insert model.uid newDev model.devList
              }
            , Cmd.none
            )

        DeleteDeveloper id ->
            ( { model | devList = Dict.remove id model.devList }, Cmd.none )
...
moderator : Maybe Int -> Dict Int Developer -> Html Msg
moderator id list =
    div []
        [ label [] [ text "Moderator: " ]
        , select
            [ on "change" (Json.map SetModerator targetValueIntDecoder) ]
            (List.map devOption (Dict.values list))
        ]
...
getDeveloperFromId : Maybe Int -> Dict Int Developer -> Maybe Developer
getDeveloperFromId maybeId devList =
    case maybeId of
        Just devId ->
            Dict.get devId devList
...
getFilteredList model =
    ...
    in
        case maybeDev1 of
            Just dev1 ->
                Dict.values model.devList
                    |> List.filter (\devN -> devN.id /= dev1.id)
...
--developerList : Dict Int Developer -> Html Msg
...
developerList devList =
    section
        []
        [ ul [] (List.map showDev (Dict.values devList)) ]
